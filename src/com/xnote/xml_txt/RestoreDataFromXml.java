...
public class RestoreDataFromXml {
	...
	public void restoreData() throws Exception {
		...
		MyLog.d(XNoteMainActivity.TAG, "RestoreDataFromXml==>XML文件中Note Item 的数量："
				+ records.size());
		int id = -1;
		for (Note tmpNote : records) {
			...
			MyLog.d(XNoteMainActivity.TAG, "RestoreDataFromXml==>isfolder:"
					+ isFolder);
			int parentFolder = tmpNote.getParentfolder();
			MyLog.d(XNoteMainActivity.TAG, "RestoreDataFromXml==>原来的parentfolder:"
					+ parentFolder);
			// 恢复记录
		...
		MyLog.d(XNoteMainActivity.TAG, "RestoreDataFromXml==>Restore finished...");
	}
}