...
public class MyDefaultHandler extends DefaultHandler {
	...
	public void characters(char[] ch, int start, int length)
			throws SAXException {
		MyLog.e(XNoteMainActivity.TAG, "^^^当节点中有换行符时,characters()函数不止执行一次^^^  "
		...
		if (mNote != null) {
			...
			if (preTag.equals("id")) {
				...
				MyLog.d(XNoteMainActivity.TAG, "读取XML文件时==>characters()函数中的数据: "
						+ strContent);
	...
	public void endElement(String uri, String localName, String qName)
			throws SAXException {
		if (!TextUtils.isEmpty(strElementData)) {
			MyLog.d(XNoteMainActivity.TAG, "从XML文件中读取到的完整的content节点的内容: "
					+ strElementData);
			MyLog.d(XNoteMainActivity.TAG, "从XML文件中读取到的完整的content节点的内容打印完毕!");
			// 保存Content到Note对象中
