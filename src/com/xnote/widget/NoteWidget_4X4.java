...
public class NoteWidget_4X4 extends AppWidgetProvider {
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
			int[] appWidgetIds) {
		MyLog.d(XNoteMainActivity.TAG, "NoteWidget_4X4==>onUpdate()");
		int c = appWidgetIds.length;
		if (c > 0) {
			for (int i = 0; i < c; i++) {
				MyLog.d(XNoteMainActivity.TAG, "NoteWidget_4X4==>要更新的AppWidget的Id: "
						+ appWidgetIds[i]);
	...
	public void onDeleted(Context context, int[] appWidgetIds) {
		MyLog.d(XNoteMainActivity.TAG, "NoteWidget_4X4==>onDeleted()");
	...
	public static void updateAppwidget(Context context,
		...
		int id = (int) prefs.getLong(EditWidgetNoteActivity.SHAREDPREF
				+ appWidgetId, -1);
		MyLog.d(XNoteMainActivity.TAG, "NoteWidget_4X4==>AppWidget中的记录在数据库中的id: "
				+ id);
		if (id != -1) {// 读取的id正常
			// 查询该记录的所有内容
			...
			MyLog.d(XNoteMainActivity.TAG, "显示的内容 ： " + mContent);
			views.setOnClickPendingIntent(R.id.widget_4x4_imageView,
					pendingIntent);
			if (views != null) {
				MyLog.d(XNoteMainActivity.TAG, "NoteWidget_4X4==>RemoteViews不为空！");
