...
import com.miles.ui.views.CircularImage;
import com.miles.ui.views.RoundedCornersImage;
...
public class PublicActivityAdapter extends BaseAdapter {
 ...
 // 控制图像显示
    ...
    public static final String TEXT_FORMAT = "<font color='#1479ad'>%s</font> 对 <font color='#1479ad'><b>%s</b></font>发表了笔记："
    											+"<font color='#1479ad'><b>%s</b></font></br>我在:<font color='#1479ad'><b>%s</b></font>";
    ...
    public PublicActivityAdapter(Context context,BookBean book, List<ActivityMessage> messages) {
        super();
		...
		options = new DisplayImageOptions.Builder()
				...
				.showImageOnFail(R.drawable.head1).cacheInMemory()
    ...
    public List<ActivityMessage> getMsgs() {
		return msgs;
	}



	public void setMsgs(List<ActivityMessage> msgs) {
		this.msgs = msgs;
	}
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null || (holder = (ViewHolder) convertView.getTag()).flag != position) {
            ...
            if (position == 0) {
                ...
                //书图像
                ImageView avatar = (CircularImage) convertView
                ...
                TextView bookNameText = (TextView)convertView.findViewById(R.id.cover_clock);
                ...
                if(book.getmISBN().equals("000"))
                {
            		...
            		if(BookApplication.userAvator != null && !BookApplication.userAvator.equals(""))
            		{
            			...
            			if(avator.exists())
            			{
            				ImageLoader.getInstance().displayImage(Uri.fromFile(avator).toString(), avatar,
            						options);
            		...
            		bookNameText.setText(BookApplication.userName+" "+BookApplication.userNum);
                ...
                else
                {
                	ImageLoader.getInstance().displayImage(
                		book.getmPicUrl(), avatar,
    					options);

                	bookNameText.setText(book.getmTitle());
               ...
               // 
                ...
                if(message.getAuthorPic() != null && !message.getAuthorPic().equals(""))
                {
                	 File temp = new File(Environment.getExternalStorageDirectory(),
                     		message.getAuthorPic());
                	ImageLoader.getInstance().displayImage(Uri.fromFile(temp).toString(), authorView,
						options);
                }
                else
                {
                ...
                if (ActivityMessage.MESSAGE_TYPE_TEXT == type) {
                    ...
                    String txtstr = String.format(TEXT_FORMAT, message.getAuthorName(),
                            message.getBookName(),message.getBody(),message.getStoreName());
